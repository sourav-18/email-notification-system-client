...
import { removeUserDataFormLocalStorage } from '../../utils/fun.util';
import { useNavigate } from 'react-router-dom';
import { AllState } from '../../context/Context';
import constantData from '../../utils/constant.util';

export default function LogoutProfile({ name = "name", userImage = profileLogo }) {
  ...
  const items = [
    { label: "Sign Out", icon: LogOut, color: "text-rose-500", glow: "group-hover:shadow-[0_0_15px_rgba(244,63,94,0.3)]" },
  ];

  return (
    <div className="fixed top-8 right-8 z-[90]" ref={menuRef}>
      <Confirm isOpen={confirmPopupOpen} onClose={() => handleClosePopup()} onConfirm={() => handleLogout()} />
      <div className="flex flex-col items-end">
        ...
        {isOpen && (
          <div className="absolute top-16 right-0 w-56 origin-top-right">
            <div className="relative bg-slate-900/90 border border-white/10 rounded-[28px] shadow-[0_40px_80px_-15px_rgba(0,0,0,0.9)] overflow-hidden animate-in fade-in zoom-in-95 slide-in-from-top-4 duration-300">
              ...
              <div className="px-6 py-3 border-b border-white/[0.05]">
                <div className="flex items-center justify-center gap-1.5 mt-0.5">
                  ...
                  <h3 className="text-[14px] font-bold text-white text-center tracking-tight uppercase">{adminProfile?"Admin":"Organization"}</h3>
                </div>
