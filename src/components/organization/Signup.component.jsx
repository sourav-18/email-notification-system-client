...
import { organizationSignup } from '../../services/auth.service';
...
const CompactSignup = () => {
    ...
    const handleSignup = async () => {
        ...
        if (password !== confirmPassword) {
            return sendAlert("Password and ConfirmPassword should be match");
        }
        const signupData = {
            name: name,
            emailId: email,
        ...
        const apiRes = await organizationSignup(signupData);
        ...
        setUserDataInLocalStorage({
            ...
            secretKey: apiRes.data.secretKey,
    ...
    return (
        <div className="flex min-h-screen items-center justify-center p-4 antialiased">
            ...
            <div className="relative w-full max-w-[415px] bg-[#111827] border border-white/10 rounded-[15px] p-6 shadow-[0_20px_50px_rgba(0,0,0,0.4)]">
                ...
                <div className="flex flex-col items-center mb-6 text-center">
                    ...
                    <h2 className="text-[17px] font-bold text-white tracking-tight">Create Organization</h2>
                </div>

                <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>

                    <div className="relative group">
                        <ALargeSmall className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-cyan-400 transition-colors" size={14} />
                        <input
                            type="name"
                            placeholder="name"
                            value={name}
                    ...
                    >
                        <span className="flex items-center justify-center gap-1">
                            Complete Setup <ChevronRight size={14} className="group-hover:translate-x-0.5 transition-transform" />
                        </span>
                    </button>

                    {/* Minimal Footer */}
                    <p className="text-center text-[11px] text-gray-500 mt-4 tracking-wide font-medium">
                        Already a member? <Link to="/login" className="text-cyan-500 hover:underline">Sign In</Link>
                    </p>
                </form>
